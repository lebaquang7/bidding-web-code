...
public class ManualClientTestRunner {
    ...
    public static void testControllers() {
        try {
            ...
            if (loginCtrl != null && registerCtrl != null && mainCtrl != null) {
                System.out.println("[PASSED] TC01: Cấu trúc liên kết các lớp Controller (Đăng nhập, Đăng ký, Menu) khớp 100%.");
            }
        } catch (Exception e) {
            System.err.println("[FAILED] TC01: Lỗi cấu trúc lớp điều khiển hoặc thiếu Class: " + e.getMessage());
        }
    }
    ...
    public static void testHandlers() {
        try {
            ...
            System.err.println("[FAILED] TC02: Lỗi khởi tạo thực thể nghiệp vụ Models: " + e.getMessage());
        }
    }
        ...
        for (String path : fxmlPaths) {
            ...
            try (InputStream is = ManualClientTestRunner.class.getResourceAsStream(path)) {
                if (is != null) {
                    ...
                    System.out.println("[INFO] Đường dẫn kiểm tra nội bộ: " + path + " (Cần build/update tài nguyên hệ thống)");
                }
            } catch (Exception e) {
            }
        }

        System.out.println("[PASSED] TC03: Kiểm tra liên kết thư mục tài nguyên giao diện cơ bản hoàn tất.");
    }
}