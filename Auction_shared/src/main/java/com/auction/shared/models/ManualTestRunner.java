...
public class ManualTestRunner {
    ...
    public static void testArtFactoryCreation() {
        ...
        Item item = artFactory.createItem("Tranh Đêm đầy sao", "Bản sao phục dựng", BigDecimal.valueOf(500.0), BigDecimal.valueOf(500.0));
    ...
    public static void testElectronicsFactoryCreation() {
        ...
        Item item = elecFactory.createItem("iPhone 17 Pro Max", "Máy trần 99%", BigDecimal.valueOf(1000.0), BigDecimal.valueOf(1000.0));

        if (item instanceof Electronics && item.getId() != null && !item.getId().isEmpty()) {
            System.out.println("[PASSED] TC12: ElectronicsFactory tạo thành công. UUID tự động sinh: " + item.getId());
        } else {
    ...
    public static void testVehicleFactoryCreation() {
        ...
        Item item = vehicleFactory.createItem("Xe điện Vinfast", "Xe chính chủ", BigDecimal.valueOf(200.0), BigDecimal.valueOf(200.0));

        if (item instanceof Vehicle && item.getStartingPrice().compareTo(BigDecimal.valueOf(200.0)) == 0) {
    ...
    public static void testNetworkRequest() {
        try {
            NetworkRequest request = new NetworkRequest(NetworkRequest.requestType.values()[0], "Dữ liệu mẫu");
