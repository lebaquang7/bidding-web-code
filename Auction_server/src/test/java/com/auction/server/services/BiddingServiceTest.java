...
public class BiddingServiceTest {
    ...
    void testPlaceBid_Success() {
        ...
        Mockito.when(mockItem.getSellerId()).thenReturn("seller_001"); // Giả lập tránh lỗi NullPointerException
        ...
        mockedDbConfig.when(() -> DatabaseConfig.executeBidTransaction(Mockito.eq("item1"), Mockito.anyString(), Mockito.any())).thenReturn(true);

        // Thực thi test
        BidStatus.bidStatus result = BiddingService.placeBid("item1", "user_new", new BigDecimal("120.00"));
    ...
    void testConcurrentBidding_Safety() throws InterruptedException {
        ...
        for (int i = 0; i < numberOfThreads; i++) {
            ...
            service.submit(() -> {
                ...
                try (MockedStatic<DatabaseConfig> threadMockedDb = Mockito.mockStatic(DatabaseConfig.class, Mockito.RETURNS_DEFAULTS)) {

                    // Giả lập dữ liệu cho luồng này
                    threadMockedDb.when(() -> DatabaseConfig.getItemById("concurrency_id")).thenReturn(mockItem);
                    threadMockedDb.when(() -> DatabaseConfig.isAuctionRunningInDB("concurrency_id")).thenReturn(true);

                    // Luồng nào chạy lệnh này đầu tiên (successCount == 0) thì DB trả về true, các luồng sau bị false
                    threadMockedDb.when(() -> DatabaseConfig.executeBidTransaction(Mockito.eq("concurrency_id"), Mockito.anyString(), Mockito.any()))
                            .thenAnswer(invocation -> successCount.get() == 0);

                    readyLatch.countDown();
                    startLatch.await();

                    BidStatus.bidStatus status = BiddingService.placeBid("concurrency_id", bidderId, new BigDecimal("150.00"));
