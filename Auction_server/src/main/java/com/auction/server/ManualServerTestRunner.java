...
public class ManualServerTestRunner {
  ...
  public static void testAuctionManager() {
    try {
      ...
      Auction auction =
          new Auction(
              101, null, BigDecimal.valueOf(500.0), seller, LocalDateTime.now(), LocalDateTime.MAX);
      ...
      try {
        ...
        java.lang.reflect.Constructor<AuctionManager> constructor =
            AuctionManager.class.getDeclaredConstructor();
      ...
      if (manager != null) {
        ...
        System.out.println(
            "[PASSED] TC05: Cấu trúc logic xử lý của AuctionManager hoạt động bình thường.");
      }

    } catch (Exception e) {
      System.err.println(
          "[FAILED] TC04 & TC05: Lỗi kiểm thử bộ quản lý AuctionManager: " + e.getMessage());
    }
  }
  ...
  public static void testDatabaseConfig() {
    try {
      ...
      try {
        Connection conn =
            (Connection) dbConfig.getClass().getMethod("getConnection").invoke(dbConfig);
        if (conn != null) {
          System.out.println("[PASSED] TC07: Kết nối thử nghiệm tới Database thành công!");
        }
      } catch (Exception ex) {
        System.out.println(
            "[INFO] TC07: Chưa bật dịch vụ SQL/XAMPP (Hệ thống bắt ngoại lệ an toàn, code không crash).");
      }

    } catch (Exception e) {
      System.err.println(
          "[FAILED] TC06 & TC07: Lỗi cấu trúc lớp DatabaseConfig: " + e.getMessage());
    }
  }
  ...
  public static void testServerComponent() {
    try {
      ...
      if (server != null) {
        System.out.println(
            "[PASSED] TC08: Thành phần mạng lõi MainServer định hình cấu trúc thành công.");
      }
    } catch (Exception e) {
      System.err.println(
          "[FAILED] TC08: Lỗi khởi tạo thành phần kết nối mạng Socket: " + e.getMessage());
    }
  }
}
